...
export function CaneTypeSummaryPanel({ result, showSource = true }: { result: DataResult<CaneTypeSummary[]>; showSource?: boolean }) {
    ...
    <section className="card full-span cane-summary-panel">
      <div className="card-title-row">
        <div>
          ...
          <p className="muted">สรุปพื้นที่ตามประเภทอ้อยจากข้อมูลกิจกรรม เพื่อดูสัดส่วนที่ใช้ประกอบคาร์บอนเครดิตและคาร์บอนฟุตพริ้นท์</p>
        </div>
        {showSource && <SourceBadge source={result.source} meta={result.meta} />}
      </div>

      <div className="cane-kpi-grid">
        <div><strong>{planted?.percent.toFixed(1) ?? "0.0"}%</strong><span>อ้อยปลูก</span></div>
        <div><strong>{ratoon?.percent.toFixed(1) ?? "0.0"}%</strong><span>อ้อยตอ</span></div>
        <div><strong>{fallow?.percent.toFixed(1) ?? "0.0"}%</strong><span>พื้นที่พักดิน</span></div>
      ...
      <div className="cane-legend-grid">
        {result.data.map((item, index) => (
          <div key={item.name}>
            ...
            <small>{formatRai(item.areaRai)} ไร่ · {item.percent.toFixed(1)}%{item.co2eTotal != null ? ` · ${item.co2eTotal.toLocaleString()} tCO2e` : ""}</small>
          </div>
        ))}
      </div>
